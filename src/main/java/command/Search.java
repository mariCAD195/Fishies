...
public class Search extends Command {
    ...
    public String execute() {
        if(!map.getLocations().get(larry.getCurrentLocation().getName().toLowerCase()).getAquariums().isEmpty()) {
            ...
            if (map.getLocations().get(larry.getCurrentLocation().getName().toLowerCase()).getAquariums().contains(input)) {
                ...
                switch (input2) {
                        ...
                        for (int i = 0; i < map.getAquariums().get(input).getActiveFish().size(); i++) {
                            if (fishExists == false) {
                                if (map.getAquariums().get(input).getActiveFish().get(i).getSpecies().trim().equalsIgnoreCase(input3)) {
                                    journal.takePhoto(input3, story, larry);
                                    fishExists = true;
                                    return "";
                                }
                            }
                    ...
                    case "sign":
                        ...
                        if (!fishExists2) {
                            return "\nsorry I don't see any fish like this";
                        }
                }
            } else {
                return "\nsorry, no fish tanks like that in here";
            }
        }else{
            return "\nsorry, no fish tanks in here";
        }
        return "";
    }
